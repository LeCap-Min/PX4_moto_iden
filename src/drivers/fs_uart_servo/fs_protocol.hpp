/****************************************************************************
 *
 * Minimal FashionStar bus protocol framer — sync angle commands.
 * Mirrors FSUS_Protocol::sendSyncCommand() from the official FashionStar C++ SDK.
 *
 ****************************************************************************/
...
/**
 * MODE_SET_SERVO_ANGLE — official fastest/basic single-turn sync mode.
 * Wire layout: subcommand 8, 7 bytes per servo (id, angle, interval, power).
 * The caller selects fastest motion by setting interval_ms to zero.
 */
inline uint16_t build_sync_angle_frame(const SyncServoParam *servos, uint8_t count,
		uint8_t *out_buf, uint16_t out_cap)
{
	...
	const unsigned content_bytes = static_cast<unsigned>(3) + static_cast<unsigned>(count) * 7U;
	const unsigned single_byte_len_field = (content_bytes < 255U) ? 1U : 3U;
	const unsigned wire_len = 2U + 1U + single_byte_len_field + content_bytes + 1U;
...
/**
 * MODE_SET_SERVO_ANGLE_MTURN — official fastest/basic multi-turn sync mode.
 * Wire layout: subcommand 13, 11 bytes per servo
 * (id, int32 angle, uint32 interval, power). The PX4 parameter is limited to
 * 2000 ms, so SyncServoParam's uint16 interval is sufficient here.
 */
inline uint16_t build_sync_angle_mturn_frame(const SyncServoParam *servos, uint8_t count,
		uint8_t *out_buf, uint16_t out_cap)
{
	if (servos == nullptr || out_buf == nullptr || count == 0) {
		return 0;
	...
	for (uint8_t si = 0; si < count; si++) {
		...
		const uint32_t angle_bits = static_cast<uint32_t>(angle_int10);
		const uint32_t interval_multi = static_cast<uint32_t>(servos[si].interval_ms);
		out_buf[idx++] = servos[si].id;
		out_buf[idx++] = static_cast<uint8_t>(angle_bits & 0xFFU);
		out_buf[idx++] = static_cast<uint8_t>((angle_bits >> 8) & 0xFFU);
		out_buf[idx++] = static_cast<uint8_t>((angle_bits >> 16) & 0xFFU);
		out_buf[idx++] = static_cast<uint8_t>((angle_bits >> 24) & 0xFFU);
...
public:
	...
	Result feed(uint8_t b, Frame &out)
	{
		...
		case State::Wait_h1:
			...
			/* A lone 0x05 is common in echoed request payloads (for example servo ID 5).
			 * It is only noise while searching for 0x05 0x1c, not a failed response. */
			return Result::Need_more;
	...
	case State::Wait_content:
			/* feed() may be called with a different Frame object for every byte.
			 * Keep the in-progress payload in the parser, then copy it to `out`
			 * only when the checksum completes the frame. */
			_content[_idx++] = b;
			_sum += b;
		...
		case State::Wait_checksum: {
				...
				for (uint8_t i = 0; i < _content_size; i++) {
					out.content[i] = _content[i];
				}
