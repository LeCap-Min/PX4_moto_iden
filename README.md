# PX4 电机 / 舵机辨识（v1.16.0 分支）

本目录是 **PX4 v1.16.0** 上的辨识固件，与同级的 `PX4_moto_iden`（基于 v1.13.3）分开保存。本地 `/home/lab/PX4/1.16.0` 官方备份**不会被修改**。

本分支在 [PX4 Autopilot v1.16.0](https://github.com/PX4/PX4-Autopilot/tree/v1.16.0) 上迁移了**多旋翼单电机推力辨识**、**FashionStar UART 总线舵机自适应扫频辨识**与 **PWM 舵机自适应扫频辨识**。电机辨识保留 1.16 的 **DShot 回传**（双向 DShot RPM，以及可选 UART 遥测），写入 `Identify_data` 与 `esc_status`。

Git 分支名：`PX4_moto_iden_1.16`。
...
```bash
git clone <本仓库 URL>
cd PX4_moto_iden_1.16
git checkout PX4_moto_iden_1.16
...
本分支已对 `px4_fmu-v5_default` 做**台架辨识专用裁剪**：保留 EKF2 姿态（`Servo_iden_data` 的 roll/pitch）、RC、USB/TELEM1 MAVLink、px4io MAIN + DShot、PWM AUX 与 `fs_uart_servo`；关闭 GPS、UAVCAN、位置控制/导航/自动模式等运行时模块。`navigator` 与 `mc_pos_control` 仅编译进固件以提供参数定义，**开机不启动**。

---

## 使用前准备

- **控制分配**：PX4 1.16 默认启动 `control_allocator`（不再使用 `SYS_CTRL_ALLOC`）。
- **电机 DShot 回传**：默认 `DSHOT_BIDIR_EN = 1`（双向 DShot，电调需支持，如 BLHeli32 / AM32）。转速写入 `esc_status` 与 `Identify_data.esc_rpm`。另可用 `DSHOT_TEL_CFG` 配置 UART 遥测（电压/电流/温度）。不支持双向时把 `DSHOT_BIDIR_EN` 改回 0。`MOT_POLE_COUNT` 需与电机磁极数一致（默认 14）。
- **日志**：`SDLOG_PROFILE` 包含 Default 时会记录 `Identify_data`、`Servo_iden_data`、`Fs_data`，并保留 1.16 的 `esc_status`（DShot 回传）。若使用 SD 卡上的 `logger_topics.txt` 自定义列表，需自行包含上述话题。
- **总线舵机辨识（IDEN_TYPE=3）**：
...
| 话题 | 说明 |
|------|------|
| **`Identify_data`** | 电机辨识：由 `control_allocator` 发布，含 `motor_idx`、`identify_active`、`cmd_norm`、`pwm_out`（DShot 时为油门计数），以及 1.16 DShot 回传：`esc_rpm` / `esc_voltage` / `esc_current` / `esc_temperature` / `esc_online` / `dshot_tel_ok` |
...
- `msg/IdentifyData.msg`、`msg/Servo_iden_data.msg`、`msg/Servo_iden_ctrl.msg`：辨识消息定义。
- `src/drivers/fs_uart_servo/`：FashionStar UART 总线舵机驱动（type 3 在发送节拍生成波形，发布 `Fs_data` / `Servo_iden_data`）。
- `msg/Fs_data.msg`：舵机驱动状态与回传数据。
- `src/drivers/dshot/`：**未改**，沿用 1.16 双向 DShot / UART 遥测，经 `esc_status` 进入电机辨识日志。
- `src/modules/logger/logged_topics.cpp`：默认记录 `Identify_data`、`Servo_iden_data`、`Fs_data`，并保留 `esc_status`。
...
- `ROMFS/px4fmu_common/init.d/rc.mc_apps`：手飞控制链 + `control_allocator`；不启动 navigator / mc_pos_control / flight_mode_manager。
- `ROMFS/px4fmu_common/init.d/rc.mc_defaults`：默认 `DSHOT_BIDIR_EN=1`。
